...
class Category(models.Model):
    ...
    def save(self):
        if not self.image_origin:
            ...
            self.image_origin = InMemoryUploadedFile(output,'ImageField', "%s.jpg" %self.image_origin.name.split('.')[0], IMAGE_FOLDER+'Category/image/jpeg', sys.getsizeof(output), None)
...
class Product(models.Model):
    ...
    comments=models.ManyToManyField("dashboard.Comment", verbose_name=_("نظرات کاربرات"),blank=True)
    tags=models.ManyToManyField("dashboard.Tag", verbose_name=_("برچسب ها"),blank=True)
    metadatas=models.ManyToManyField("dashboard.MetaData", verbose_name=_("کلمات کلیدی سئو"),blank=True)
    def get_price(self):
...
class Supplier(models.Model):
    region = models.ForeignKey("dashboard.region", verbose_name=_("region"), on_delete=models.PROTECT)
